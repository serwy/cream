...
function cpcMTO(CPC_def)
{
	...
	for (var i = 0; i < CPC_def.length; i++) {
		var level = cpc.level;
		for(var tmp = 0; tmp < CPC_def[i]; tmp++) {
			level = level.next;
		}

	
		if (level.perf.toUpperCase() == "REDUCED") {
			// add tick marks to the MTO totals
			MTO[0] += parseInt(level.parent.man);		
			MTO[1] += parseInt(level.parent.tech);
			MTO[2] += parseInt(level.parent.org);
		}
...
var CPC_HEAD = null;	// the actual cpc's
var CPC_LEVEL = null;	// the analysis level array
...
function loadCPC(filename)
{
	...
	CPC_NOTES = new Array(lengthOf(CPC_HEAD));  // pointers into CPC data stru
